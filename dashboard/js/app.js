/* =========================================================
   RESCUE BAND — 애플리케이션 로직
   순수 JavaScript · 해시 라우팅 · 프레임워크 없음
   ========================================================= */
(function () {
  'use strict';
  ...
  const ROUTES = {
    dashboard: { tab: 'dashboard', render: viewDashboard },
    crew:      { tab: 'crew',      render: viewCrew },
    history:   { tab: 'history',   render: viewHistory },
    alerts:    { tab: 'alerts',    render: viewAlerts },
    settings:  { tab: null,        render: viewSettings }
  };
  ...
  // 주의 필요: 위험 상태 · 배터리 20% 이하 · 심박 120 이상
  ...
  function viewDashboard() {
    ...
    const activeHtml = active.length ? active.map(h => `
      ...
      </div>`).join('') : `<div class="card empty"><h3>진행 중인 출동 없음</h3><p>새 출동 지령이 여기에 표시됩니다.</p></div>`;

    const attention = DB.crew.filter(needsAttention);
    const attentionHtml = attention.map(c => crewCardHtml(c, true)).join('')
  ...
  function viewCrew() {
    ...
    const cards = list.map(c => crewCardHtml(c, false)).join('')
  ...
  function render() {
    ...
    $$('.tabbar__item').forEach(n => n.classList.toggle('is-active', n.getAttribute('data-route') === route.tab));
    if (key !== 'crew') $('#view').scrollTop = 0;
    closeModal();
  }

  function updateBadges() {
    const n = state.alerts.filter(a => !a.read).length;
    ['#bellBadge', '#tabAlertBadge'].forEach(sel => {
      const b = $(sel);
